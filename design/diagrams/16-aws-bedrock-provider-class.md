...
classDiagram
    ...
    note for IdentifiedProviderNeutralModelRequest "Request construction, invocation, response validation, decoding and schema validation have fake-provider evidence without capacity fields or gates. APIs own size limits; actual workflow token usage is budgeted. Native YAML preparation retains one workflow-owned request across steps. Provider-call integration and native-schema representability remain pending"
