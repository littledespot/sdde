...
| JSON or schema retry | The original task content plus the selected protocol prompt, the exact rejected response as untrusted evidence, the decoder position and object/key context or schema JSON Pointer, and the original complete schema. Schema corrections add the exact expected schema node with its JSON Pointer and parent/value scope, preserving alternatives and bounds. No candidate examples are generated. The protocol prompt prohibits reconsidering business meaning. Serialization emits the same universal framing instruction once; it does not accumulate in retained retry content. |
