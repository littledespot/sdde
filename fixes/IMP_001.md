...
The latest approved [R38 execution](FIX_001.md#40-brief-generation-schema-exhaustion-after-approved-recovery-delivery)
...
is implied.** R38's first brief response and both corrections are byte-identical
...
1. **Cover complete generation failure and recovery through existing owners.**
   Add the preceding successful summary correction and accepted global slices,
   then brief root-provenance/wrapper rejection → unchanged correction → exhaustion.
...
2. **Assess the existing shared request representation before changing it.**
   The selected schema already narrows this assignment to brief or clarification.
   Its root correction outline lists the three child objects but leaves their
   required `value`/`provenance` fields to the full schema. Compare a concise,
   schema-derived description of the relevant nested shape using the existing
   `model_schema_projection`/`model_protocol_retry` owners; retain one complete
   authoritative schema and do not hard-code brief field names or auto-move evidence.
   Separately assess removing unrelated generation-unit guidance via the existing
   assignment/guidance owner. Measure request bytes/tokens and cover other units,
   selected repairs and clarification alternatives. Do not assume more prompting
   or another decomposition layer improves compliance. If the useful change
   exceeds the approved §22.6 projection or composition contract, document that
   focused decision before implementation; no amendment is approved by this entry.
3. **Reverify and retain honest readiness.** Run affected request/schema/generation
   ...
   live publication/scoring still unmet. Prepare one concrete Chunk 19 run only
...
and [R38 generation failure](FIX_001.md#40-brief-generation-schema-exhaustion-after-approved-recovery-delivery).**
...
- **Next: Chunk 18 — R38 generation conformance.** The [follow-up](#r38-follow-up--generation-conformance)
  scopes complete failure/recovery regression coverage and assessment of concise
  shared shape/assignment guidance. The live failure establishes no mismatch
  between the schema and native brief contract and authorizes no local exception.
...
- **Chunk 18 — R36 logical slices implemented and verified offline.**
  ...
  persisted readback; R38 reaches validated global assembly, while live
  publication/scoring remains unmet.
...
**Next: complete [R38 conformance work](#r38-follow-up--generation-conformance).**
...
- [ ] Complete chunk 18. **Depends on:** 01–15, 17; 16 if it changed shared contracts.
  **Latest delivery:** [R37](#r37-shared-recovery-and-persisted-contract-delivery)
  implements the approved shared missing-answer continuation and persisted
  extraction-contract binding. Its current verification supersedes the earlier
  open decisions below; the published/scored baseline remains Chunk 19.
...
- [ ] Complete chunk 19. **Depends on:** 18 and explicit approval for this run.
  ...
  One further explicitly approved invocation ran as R38 and failed in brief
  generation; publication and grading were `not_run`. Address its
  [conformance follow-up](#r38-follow-up--generation-conformance) before proposing
  another run. Each invocation requires its own approval after verification;
  prior approvals do not carry forward.
