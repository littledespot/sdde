...
Phase 3 is incomplete: 08–10 remain open for the bounded work in the
[architecture follow-up](#phase-3-architecture-audit-follow-up); 11–12 are delivered.
...
C3 eligibility ownership is consolidated; chunks 11–12 are delivered.
...
- [ ] Chunk 08 partially delivered; the 15 September review leaves shared
  feasibility/equivalence work under C1/C2; C3 is consolidated.
...
supersedes the blanket completion claim. C3 is now consolidated; C1/C2 remain below.
...
| --- | --- | --- |
| 08 — Native facts and authorization | Partial: C1/C2; C3 consolidated | Shared `typed_text.Dependencies` includes grammar bindings, naming rules, source inputs and passive occurrences. Reconciliation captures flattened typed lineage once; specification generation and coverage reuse provenance dependencies. Runtime toolchain identity is checked before capture. Snapshots no longer depend on model packets. Tests reject changed grammar names, stale policy identity and broken history before dispatch/merge, while presentation-only formatting leaves native facts unchanged. Existing exact CAS, owner/revision/origin, wrong-kind, extra-field and allocation-failure tests remain. |
...
20’s scheduling dependency on the first live baseline. C1/C2 and the live-baseline
criteria remain open; this work changes no accepted policy.
...
policy still requires removal through the closure work above. C3’s duplicate
eligibility filters were removed on 15 September; later-phase removals remain pending:
...
F1/F4 failures. C3 ownership cleanup and chunk 20 are now delivered. Current
chunk status and closure work remain solely in the rollout; chunks 13–19 and 21
are still pending.
...
C3 and chunk 20 were subsequently completed and verified offline; C1/C2 remain
open. See the [closure evidence](#c3-and-chunk-20-validation--15-september-2026).
