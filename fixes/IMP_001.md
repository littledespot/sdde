...
**20 September 2026; evidence/plan update only.** The [R45 analysis](FIX_001.md#47-brief-schema-exhaustion-despite-child-object-guidance)
...
1. **Reuse conformance coverage and retained requests.** Extend the existing brief
   recurrence/recovery cases only where R45 adds evidence: assert the actual child
   required-field outline and complete selected schema on both corrections; cover
   wrapper recovery followed by invalid token/citation provenance, and then a
   fully valid recovery. Preserve earlier accepted units, origins, the single
   unresolved request's allowance and exact token accounting. Failure must still
   prevent publication. Include unrelated attributed-value shapes; do not add a
   Hello World rule or duplicate the existing exhaustion fixture.
2. **Finish the previously proposed assignment-guidance simplification.** Use
...
3. **Prepare a controlled comparison under §28.8 before expanding the design.**
...
4. **Assess further configured slicing only if measured failures warrant it.**
...
Implementation must run full `zig build verify` and packaging checks. Chunk 19
...
This documentation review made no code changes and ran no model or workflow.
