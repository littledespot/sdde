...
**Next:** retain the offline delivery and finish the shared response-contract/model
conformance assessment before selecting an explicitly scoped comparison. A successful
published, scored baseline remains unproven; retry containment cannot guarantee
model compliance. Wider regenerated-subject recovery requires an explicit stable
mapping contract before implementation.
