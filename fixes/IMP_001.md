...
The latest [R31 review](FIX_001.md#34-source-review-protocol-exhaustion-live-run-review)
...
updates the existing shared instruction and failure/recovery coverage. Remaining
work is Chunk 13 detail/source-preservation guidance and Chunk 14 dependency
renewal, followed by combined readiness and an approved live run.
...
The root result schema is supplied once, but correction also serializes its expected
subtree in full. R31 call 13 repeats **2,361 bytes / 554 estimated tokens** beside a
**2,503-byte / 587-token** root schema; reconciliation corrections repeat a
**6,408-byte / 1,528-token** subtree. This is duplication from the same compiled
owner, not a conflicting contract. No context clipping is established.

Assess replacing that redundant copy with a canonical locator and concise
structural facts derived from the **same compiled schema**, retaining exact error
path/scope and every applicable alternative/constraint in the one authoritative
schema. Prefer the smallest representation that explains the violated shape;
measure it on missing tags, unknown/missing properties, selected replacement
schemas and unrelated collections. Do not hand-maintain response examples,
flatten only review entries, infer ambiguous field moves or create a parallel
schema registry. Do not build an all-errors framework merely for this run.

**Focused decision required before changing that representation:**
[§22.6](../design/contracts/22-repair.md#226-unparseable-output) currently requires
the exact expected node and constraints in correction guidance. The existing JSON
Pointer renderer describes candidate locations; it is not a schema locator, and
no compact structural-fact projection exists. Record/approve the narrow amendment
before replacing that contract. This documentation review does not approve it or
claim the smaller representation will make the configured model reliable.
...
and [R31 follow-up](#r31-follow-up--complete-response-protocol-correction).**
...
review deliveries stand; chunk 14 independently blocks full verification.
...
- **Chunk 18 complete-response protocol correction/conformance delivered.**
  ...
  Next is Chunk 13 detail/source-preservation guidance and Chunk 14 dependency
  renewal. Schema-subtree reduction remains a separate representation decision.
...
- **Chunk 13 — source/principle retry identity delivered.** Source initial response
  ...
  complete validation and actual usage. Ordinary request limits retain operation
  scope; atomic repairs now use validated defect progress. Full verification remains
  blocked by chunk 14.
- **Chunk 14 — current versus historical dependencies.** Resolve the existing
  [shared lineage amendment](#shared-lineage-decision-required), then implement
  renewal through the existing repair/runner owners. Preserve historical evidence
  and unaffected source/policy freshness; invalidate and completely rebuild every
  affected dependent. The earlier minimal-target approval remains valid; it does
  not approve this unresolved frontier rule. Keep stale/mixed/incomplete generic
  dependency cases as negative regressions.
...
**Chunk 18 — concrete comparison, not another prompt patch.** Reuse the native
...
- [ ] Complete chunk 18. **Depends on:** 01–15, 17; 16 if it changed shared contracts.
  ...
  R30's Chunk 13 detail-guidance work remains outstanding.
  ...
  Complete readiness also awaits chunk 14's shared lineage decision and passing
  complete verification. The [current architecture priorities](#architecture-readiness-follow-up--shared-contracts-and-composition)
