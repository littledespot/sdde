...
- [Current architecture readiness priorities](#architecture-readiness-follow-up--shared-contracts-and-composition)
...
The latest [R28 review](FIX_001.md#28-unchanged-entity-evidence-repairs-live-run-review)
...
Chunk 13 closure requires source/principle retry-composition coverage and a passing
full check; the known chunk 14 lineage blocker remains. Plan consumption and
clarification-answer work remain separate.
...
This is an implementation proposal, not a design amendment or live-run approval.
Earlier mechanical review deliveries stand. Chunk 13's newly composed source/policy
path needs additional work; chunk 14 independently blocks full verification.

#### 1. Close shared recovery composition before extending the workflow

- **Chunk 13 — source/principle retry identity.** Post-generation source and
  principle review reuse the same compiled accounting operation with limit 1.
  Source initial response plus one successful protocol correction exhausts its two
  executions before the first principle call. The repair account is likewise shared.
  Add the combined sequence to the existing production-runner fixture, including
  source and policy corrections/repairs, unchanged repairs, real exhaustion,
  no-policy inputs and unrelated assignments. Verify exact request origins,
  full validation, siblings and actual usage, not just final status.
  Prefer distinct compiled operation identities reusing existing review/request
  implementations where the accepted graph permits it. Document intended allowance
  before changing composition. Do not reset counters or raise a caller's limits.
  Changing the runner's per-operation-instance scope requires an explicit amendment
  to [ADR 0005](../design/decisions/0005-workflow-defined-operations.md)/§12.3.
...
- **Joint constraint:** the graph already expands to 512/512 operations. Distinct
  review identities must fit the accepted representation or require a focused
  composition decision; adding another expanded request subgraph is not free.
  Preserve low-level operations and runner-owned coordination. Hidden child calls
  inside an action, a Spec-specific retry counter or a raised ceiling are not the
  architectural correction. Run targeted tests and full `zig build verify --summary
  all` after implementation, with native packaging checks where affected.
...
persisted admission. Compare request/schema tokens and total call cost across
the retained omission, foreign-selection, applicability and unchanged-repair cases,
plus unrelated requirements. Native code must not choose citations, verdicts or
semantic applicability just to make a response admissible.
...
- Seventeen model-request instances expand to 306 of the 512 operations. Reusable
  YAML already removes authored duplication, but does not reduce expanded size.
  If the retry-identity correction cannot fit through existing composition, make a
  focused request-subgraph proposal against ADRs 0012/0013 that preserves detailed
  calls, operation identity, failure routing, accounting and observability. Do not
  introduce another dispatcher, runtime graph authority or provider-calling action.
...
chunk 21; rubric calibration remains separately incomplete. A real clarification
pause is a correct outcome for missing authority, not published/scored completion.
Chunk 15's [actionable-question work](#r27-clarification-preparation-and-consolidation)
remains required; matching wording cannot merge distinct answer authority.
...
source/principle retry collision is established by code inspection and still needs
its combined regression. Chunks 13–15, 18 and 19 remain open for their stated scope.
...
- [ ] Complete expanded chunk 13. **Depends on:** 00's classification decision, 04, 06.
  ...
  combined correction/recovery coverage and an authority-preserving correction
  remain required. Full verification is also blocked by chunk 14's shared-lineage
  case. Earlier mechanical diagnostic/regression delivery remains valid.
...
- [ ] Complete chunk 18. **Depends on:** 01–15, 17; 16 if it changed shared contracts.
  ...
  are now recorded. Complete readiness awaits chunk 13's source/principle retry
  composition correction, chunk 14's shared lineage decision and passing complete
  verification. The [current architecture priorities](#architecture-readiness-follow-up--shared-contracts-and-composition)
