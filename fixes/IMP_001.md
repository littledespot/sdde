...
- [R34 next: configured response decomposition](#r34-follow-up--configured-response-decomposition)
...
The latest [approved live check](FIX_001.md#36-post-r32-live-verification--bounded-protocol-rejection)
...
remains open. Offline verification is **126/126 steps, 1,537/1,537 tests**.
...
over schema-text reduction. The design is documented, not implemented or live-proven.
...
**Design documented, 19 September 2026; implementation pending.** The user requested
generic actions/orchestrators, configuration-supplied JSON shapes, further logical
decomposition and reconstitution without LLM prompts. They accept higher input-token
usage and explicitly permit increasing the arbitrary finite graph limit. No code
change or live execution was requested in this documentation task.

**Single design authority:** [ADR 0016](../design/decisions/0016-configured-json-response-composition.md),
linked from the affected model, pipeline, orchestration, extraction, repair and
testing contracts. Its JSON encoding is proposed; the executable currently has no
composition-resource compiler or generic part assembly. [The audit](FIX_001.md#configured-decomposition-assessment--19-september-2026)
records the source-owner evidence. This plan does not add another contract owner.

**Implementation sequence — Chunk 18, with extraction integration:**

1. **Compile configured partitions.** Through the existing workflow resource and
   result-schema owners, close/validate the proposed composition resource and
   derive exact part schemas from one complete schema. Implement generic structural
   selection and placement, not extraction-property branches. Prove complete,
   disjoint per-variant ownership, dependency order and rejection of unsupported
   partitions. Keep optional containers and arrays whole initially. Extend the
   existing pointer and request-association owners; neither raw aliases nor matching
   JSON shapes authorize another part. Shared schema definitions do not merge distinct
   destination values/producers. Preserve detailed operations and reusable YAML subgraphs.
   Registry cloning must retain exact bindings to its canonical result-schema owner.
2. **Integrate the existing runner and envelope.** Bind distinct compiled part
   requests, retain actual origin/dependency evidence, and assemble without prompts
   or provider capability. Reuse/refactor the schema validator for the assembled
   candidate without pretending it is a provider response. Require matching successful
   request closure before retaining a part through the existing handoff/lifecycle
   owners. Transfer immutable evidence and retire staging in the same runner delta
   that installs the unvalidated candidate. Keep one current complete candidate;
   history cannot overwrite repaired siblings. Retain all existing retry/usage,
   native-parent and cleanup owners. Separate the stable native candidate association
   from per-value origins: both existing extraction repair owners currently put
   `entry.origin` in their defect scope. Reassembly/producer changes must not reset it.
   Once staging is retired, native domain owners alone govern repair dependencies;
   configured `requires` cannot force an extra model call for a citation repair.
   Bind exact prerequisite part placements inside the existing composition value;
   adding an unrelated sibling cannot stale them through the aggregate slot generation.
   External source/policy freshness remains with existing runner gates.
3. **Apply the first split throughout extraction.** Content/outcome plus citations,
   dependent classifications, then assembly/full validation. Update initial parsing,
   text/classification/citation provenance, native dependent rebuilding, repair
   diagnostics and evidence/report consumers together. Replace the combined prompt
   with concise part-specific requests, including retained correction bases.
   Extend native classification-item and content/citation attribution; no second
   provenance map. Keep current selected-unit repairs on native candidates and
   rebuild their derived dependencies. Do not add an unused native-to-response
   reprojection/reassembly API or reinterpret `rebuild-extraction` as generation.
   Preserve the canonical persisted domain format and its readback validation.
   Record the existing §12.7 extraction-contract metadata gap explicitly; the native
   snapshot has no such named field. Its design binding now covers the composition,
   not one call. Full §12.7 readiness remains open until its canonical metadata and
   readback requirement is implemented or explicitly amended; unchanged payload
   validation cannot satisfy that requirement. No waiver is granted here.
   Feedback is an existing unimplemented feature: update its design compatibility,
   not the whole API in this slice. Future full re-extraction uses composition;
   additive/atomic repair retains its authorized selected-unit response and merge.
   Remove the superseded combined model-call path; no compatibility decoder or
   parallel extraction pipeline. Domain coupling and semantic review remain native
   responsibilities; schema composition cannot invent missing requirements.
4. **Increase finite graph capacity as approved.** Compile the complete integration
   and select a sufficient finite bound with measured memory/stack/runtime behavior.
   Current 498 plus one existing request/retirement path is at least 514 before new
   assembly steps; this estimate is not a compiled new count. Update the canonical
   constant, derived/schema capacities and at/beyond-limit tests together. No new
   approval is needed merely to exceed 512; token budgets and retry rules do not
   change. Do not create additional orchestration complexity just to retain 512.
5. **Verify complete behavior.** Follow ADR 0016's acceptance matrix: unrelated JSON
   shapes, R34 failure/recovery, retained successful parts, real per-part origins,
   incompatible branches, current-sibling preservation during native rebuilding,
   stale/foreign evidence, allocation cleanup and unchanged accounting. Include claim
   insertion → citation repair → classification replacement → native rebuilding:
   origins follow current values while retry identity stays fixed. Include the shared
   acceptance cases for closure, atomic handoff, schema-owner cloning and three-part
   prerequisite bindings; rebuilding must not duplicate token claims or introduce
   part-generation calls. Existing dependent model operations remain unchanged.
   Run targeted contract, request, pipeline, extraction and harness checks, then full `zig build verify`
   and clean native smoke. Measure output shape complexity and full request sizes;
   higher input usage is acceptable, and no size-reduction claim is required.
6. **Close live readiness.** Propose a separately approved live run with unchanged
   model/profile, retain exact-build/input provenance and actual usage, and require
   a published, scored result for Chunk 19. Smaller outputs and passing offline
   tests alone do not establish model reliability or close separate persisted-contract
   and feedback acceptance criteria.

**Scope:** generic object-subtree partitioning, with whole arrays/optional containers
and correctly bound variants. More required-object parts need configuration and explicit workflow calls,
not property-specific code. Item-by-item array generation requires a separately
established finite assignment/coverage contract. No arbitrary merger, dynamic native
DataKeys, parallel schema store, hidden dispatcher or new semantic repair policy.

**Independent work:** the measured correction-subtree optimization remains optional
and still needs its own §22.6 representation decision. The decomposition design
does not approve that different amendment. Delivered Chunk 13 detail guidance,
Chunk 14 renewal and shared retry/accounting work stand; their separate semantic
identity limitations and Chunk 15 clarification work remain recorded.

**Documentation validation:** only Markdown under `fixes/` and `design/` changed.
No runtime schema/YAML/prompt edits, verification suite or live run occurred. The
last full runtime baseline remains 126/126 steps and 1,537/1,537 tests, predating
this design; it does not verify composition or the future graph ceiling.
`git diff --check`, local linked paths and new/changed heading targets pass; the
illustrative JSON parses and Mermaid fences remain intact. Independent contract
reviews covered schema ownership and the complete candidate/provenance lifecycle.

**Final critical review:** [findings and disposition](FIX_001.md#final-composition-design-review--19-september-2026).
The design now separates staging dependencies from domain repair, stable candidate
identity from mutable attribution, and whole generation from selected-unit repair.
It rejects the unrepresented optional-presence split, requires prompt/part conformance
and fixes the assembly-to-validation handoff. No parallel validator, retry loop,
provenance authority or feedback implementation is required. The existing persisted
extraction-contract gap remains explicitly open; this documentation is not runtime
verification or permission to ignore it.
The final scope check removes the proposed native-to-response reassembly API:
implemented repairs already retain native candidates and rebuild their dependents.
Request closure, atomic handoff, schema-owner cloning and exact part prerequisites
remain required; native per-value attribution needs extension within its existing owner.
...
**Chunk 18 — separate optimization, updated 19 September 2026.** The user's later
[decomposition direction](#r34-follow-up--configured-response-decomposition) takes
priority and accepts higher input usage. This proposal remains unimplemented and
is not a prerequisite for decomposed outputs. The
[R33/R34 audit](FIX_001.md#shared-contract-assessment--19-september-2026) confirms
intact schema/evidence delivery and variable compliance for identical requests.
It also measures redundant expected-subtree text: **5,541 bytes / 1,305 estimated
tokens** for R33 generation and **6,552 bytes / 1,560 tokens** for R34 extraction,
about a third of each complete correction request. Earlier R31 review and
reconciliation corrections repeat **2,361 / 6,408 bytes**, respectively. One shared
projection owns this cross-stage repetition; there is no competing schema authority.
No context clipping or causal guarantee of improved model compliance is established.

**Focused decision required:** amend
[§22.6](../design/contracts/22-repair.md#226-unparseable-output) to permit a canonical
reference to the expected node in the single complete selected schema, with concise
diagnostic-derived structural facts, instead of serializing that node again.
All alternatives and constraints remain available in the complete schema. The
current contract explicitly requires the exact expected node in correction
guidance; [Project authority](../AGENTS.md#project-authority) requires approval
before changing it. This documentation task does not supply that approval.

**Implementation after that decision:**

1. **Reuse the compiled schema and projection owners.** Add deterministic location
   of the expected node within the exact complete schema rendered for this request.
   Keep the candidate JSON Pointer and parent/value scope distinct from that schema
   locator, including selected root alternatives. Existing candidate-path rendering
   cannot serve as a schema locator. No parallel schema registry, validator or
   independently maintained response example is needed.
2. **Replace redundant guidance through `model_protocol_retry`.** Retain the
   original task/evidence, one complete selected schema, precise rejection, concise
   derived shape facts and latest rejected response. Preserve the existing complete
   response/preserved-meaning instruction. Delete superseded subtree serialization;
   add no model call, workflow branch, retry counter or accumulating prompt history.
   Apply equally to extraction, reconciliation, generation, source/principle reviews
   and selected repair responses. A selected repair remains its authorized response
   shape; correction must never broaden it to the whole business document.
3. **Preserve admission and authority.** Provider prompt/native projections, schema
   validation, native codecs, semantic review, atomic merge and persisted validation
   retain their existing owners. Never infer a field move, choose evidence, discard
   an entry or alter meaning to force admission. Accepted structure still requires
   full domain validation, sibling preservation and current provenance. Assignment
   identities, native defect counts and cumulative actual-token enforcement stay
   unchanged.
4. **Extend existing conformance/regression suites.** Prove schema locators resolve
   to the exact bound node for nested arrays, root alternatives, missing/unknown
   properties, missing/invalid tags, escaped names, scalar bounds and selected repair
   schemas; cover allocation cleanup. Add R34's syntax → wrong nesting → copied
   schema keyword → exhaustion and R33's misplaced provenance, plus complete bounded
   recovery and unrelated contracts. Reuse the existing mixed-success/alternating-error
   runner cases to prove continuation, unchanged accounting and latest-only evidence.
   Structure-only recovery must still reject dropped requirements, invented evidence
   and incomplete review. Replace superseded full-subtree assertions rather than
   retaining two correction formats.
5. **Measure and verify.** Compare all serialized prompt/schema/packet text against
   the retained requests; record exact bytes and consistently tokenized totals, not
   an assumed saving equal to the removed subtree. Run targeted schema/candidate,
   request-workflow and harness checks, then full `zig build verify` including native
   smoke. Synchronize §12.7's stale statement that ordinary requests retain operation
   counts with the already implemented assignment-scoped §22.7 contract; this is
   documentation drift, not this run's runtime defect.
6. **Close live readiness separately.** After offline verification, propose one
   approved comparison with the same configured model/profile and target. Observe
   complete recovery, actual usage, publication and rubric scoring. Any provider
   grammar/model-profile comparison is a separately controlled proposal, not an
   automatic fallback or simultaneous change. Only a published, scored result can
   close Chunk 19; request simplification alone cannot establish that outcome.

The 19 September assessment changed only `fixes/*`; it ran no verification suite
or live model call. Chunk 13 detail guidance and Chunk 14 dependency renewal remain
delivered; semantic mapping for regenerated subjects and Chunk 15 actionable
clarifications retain their separate recorded scope.
...
Schema-subtree compression remains the separate, unapproved representation decision
above; this delivery makes no claim that prompt wording guarantees model compliance.
...
**Analysis, 18 September 2026. Items 1–2 are implemented; item 3 remains Chunk 13
work, and item 4 remains a separate policy decision.**
...
#### 4. Decide missing-final-answer recovery separately

**Proposed; not approved by this analysis.** Current F0007 requires a final text
block and makes this `response_invalid` failure non-retryable. If recovery is
selected, the smallest decision is a provider-neutral, typed missing-final-answer
case after valid wire, association and usage checks. Its explicit workflow edge
must reuse the original request, schema, selected repair/parent key, existing
attempt allowance, authorization/lease lifecycle and cumulative token guard.
Account the failed exchange before considering another call. Repeated absence
exhausts; successful recovery still requires ordinary decoding, full validation
and publication. Amend F0006/F0007, §12 and applicable request/repair continuation
contracts before implementation, with one shared normalization policy.

Never use reasoning as answer text, manufacture an empty complete response, add
hidden adapter retries, retry every `response_invalid`, switch models/modes
implicitly or loosen malformed/foreign/refused/tool/limit response handling.
Those cases remain distinct negative tests. Existing retry-progress approval did
not approve changing this provider-failure classification.

**Readiness and order:** implement 1–2, then 3. Resolve 4 explicitly if it is to be
implemented; retain Chunk 14's independent pending lineage decision and full
dependency rebuilding. Preserve R30's lost UTC and suspect unsupported findings
in Chunk 18's existing semantic conformance matrix with unrelated sufficient
sources and genuine user gaps. Do not manufacture explanations or flip findings
to pass, and do not count a genuine clarification pause as an engine error.
Use existing build targets for affected provider/request/accounting/review/report
checks, then full `verify` and native smoke. These were not rerun for this
documentation-only analysis; **1,520/1,522** remains the prior implementation result.
Chunk 16's completed transport work stands. Chunk 19 still requires an explicitly
approved live run producing engine-published, scored output; repeated/model-setting
comparisons remain Chunk 21, not a new prerequisite framework.
...
and [R32 follow-up](#r32-follow-up--shared-protocol-progress-and-recurrence).**
...
review deliveries stand. The R32 implementation removes the known Chunk 14
stale-frontier failure; full verification passes. The approved live run now
confirms bounded rejection, but publication and scoring remain unmet.
...
- **Joint constraint:** the final shared preparation/admission and independent-review
  composition compiles to **498/512 operations**. No ceiling increase, hidden child
  execution, counter reset or clarification-dataflow exception was introduced.
  Full verification and packaging evidence are recorded with the delivery below.

#### 2. Reduce model-owned mechanical choices through existing contract owners

**Next: Chunk 18's [configured response decomposition](#r34-follow-up--configured-response-decomposition).**
Use one configured schema, generic part contracts and deterministic prompt-free
assembly through existing runner owners. Preserve source evidence and independent
retry identities; input-token reduction is secondary. The user permits a verified
finite graph-limit increase. Correction-schema text reduction remains a separate
proposal; delivered retry, detail and dependency-renewal work stands.
...
- [ ] Complete expanded chunk 13. **Depends on:** 00's classification decision, 04, 06.
  ...
  Full verification is still blocked
  by chunk 14's shared-lineage case. Earlier mechanical diagnostic/regression
...
- [ ] Complete chunk 14. **Depends on:** 00, 11–13.
  ...
  invalidation; the user approved that refined contract. Automatic recovery remains
  incomplete pending the [shared lineage decision and passing regressions](#shared-lineage-decision-required).
  Blanket source-chunk replacement remains outside the approved scope.
...
- [ ] Complete chunk 15. **Depends on:** 13–14. Stable subject identity, refresh,
  ...
  reopens actionable question preparation: diagnostic prose is copied into eight
  indistinguishable questions. Multi-subject answer consolidation requires its
  own design decision; authenticated ingestion remains H-011, not a new implicit
...
- [ ] Complete chunk 18. **Depends on:** 01–15, 17; 16 if it changed shared contracts.
  ...
  is approved; successful configured-model behavior remains chunk 19's live evidence.
...
- [ ] Complete chunk 19. **Depends on:** 18 and explicit approval for this run.
  ...
  preflight. No invocation was made; chunk 14's disposition remains unresolved.
