...
The latest [R36 execution](FIX_001.md#38-global-reconciliation-syntax-exhaustion-and-progress-assessment)
...
records the user's direction: validated logical slices, deterministic assembly and
cross-slice checks through existing owners. Recovery and mechanical correctness have
improved; reliable published/scored completion remains unproven.
...
regressions. Full verification passes 1,573 tests; persisted-contract readiness is
still awaiting its focused decision. R35 itself establishes no new assembly, runner
...
delivered below; live improvement and persisted-contract readiness remain open.
...
persisted-contract and live-quality readiness remain open.

**Remaining work:** full §12.7 readiness still requires the canonical persisted
extraction-contract metadata/readback binding, or an explicit amendment. This
pre-existing gap is not closed by unchanged snapshot validation. Future feedback
...
**Analysis and user direction, 19 September 2026.**
[R36 evidence](FIX_001.md#38-global-reconciliation-syntax-exhaustion-and-progress-assessment)
confirms successful extraction/summary recovery followed by repeated malformed global
JSON. The user directs logical slicing before final reconciliation. This section
records required work; no new implementation or live result is claimed.

**Observable outcome:** each configured slice is generated, validated and retained;
accepted slices assemble without an LLM call; the complete result undergoes cross-slice
relationship, coverage and conflict validation. Rejected slices retry within their
own assignment. A later native defect repairs its authorized target and invalidates
only its actual dependents. Final reconciliation does not regenerate all accepted
data. Whole-workflow validation remains mandatory before publication.

1. **Reuse and extend existing regression coverage.** Add R36's successful extraction,
   mixed summary repair/native token insertion, successful summary correction, then
   global stray quote → missing signal brace → byte-identical correction → exhaustion.
   Cover successful syntax recovery followed by native rejection of the mixed
   business/token signal, selected repair and restored exact-token coverage. Include
   unrelated requirements and schemas, sibling/provenance preservation, independent
   retry identities, latest-response retention and unchanged cumulative accounting.
   Reuse the production runner and current protocol/native-repair fixtures; do not
   create another retry implementation or relax acceptance.
2. **Complete the shared binding contract before wiring slices.** The canonical
   reconciliation resource roots at `$defs.summary`; global requests select its
   existing named global definition. `json_composition.compile` currently derives
   parts from `canonical.root()`, so adding three global paths to YAML alone cannot
   work. Record the focused ADR 0016/§12.9 amendment for binding one existing named
   complete definition through the current schema-selection owner. Thread that exact
   binding through compilation, registry ownership transfer, preparation, admission,
   assembly/full validation and persisted-contract metadata. No copied global schema,
   model-selected schema or second registry. The user-directed slicing outcome does
   not silently add an undeclared field to the closed composition resource.
3. **Integrate logical parts through existing owners.** Assess `claim_dispositions`,
   `signals` and `conflicts` as whole-array slices, with explicit dependencies where
   projections consume dispositions. Keep the necessary source evidence available;
   input-token minimization is secondary. Apply each slice's schema and applicable
   native checks, then use the existing disposition/signal/conflict validators for
   the complete candidate. Do not manufacture empty sibling values to satisfy a
   full-candidate validator during partial validation. Use native typed facts and
   the same validators if a dependent slice requires semantic acceptance first.
4. **Complete the native provenance handoff with the workflow.**
   `CollectReconciliation` currently reads a single admitted provider result and
   initializes one origin. Reuse assembled-candidate evidence and reconciliation's
   existing `Source`/`FieldOrigin` ownership to retain each real contributor through
   collection, diagnostics, authorization, merge and revalidation. Mixed contributors
   cannot be represented by a fabricated single provider response. Keep native
   repair on native candidates; no reverse native-to-JSON reconstruction path.
   Actions remain generic over configured shapes; domain owners supply typed facts.
5. **Prove the boundary, then assess live benefit.** Test missing/conflicting parts,
   false relationship claims, cross-slice contradictions/coverage, stale prerequisites,
   successful retained siblings, dependent rebuilding, schema ownership and cleanup.
   Measure graph/request costs and run full `zig build verify` and native packaging.
   Whole arrays retain nested item shapes; further item slicing needs the separately
   defined finite assignment/identity/coverage/order contract, not model-invented lists.
   Do not claim decomposition guarantees model compliance.
...
**Remaining readiness:** resolve and implement the persisted-contract decision below,
...
#### Persisted extraction contract — focused decision required

The R35 implementation audit confirms a design/implementation mismatch independent
of response correction. §12.7 requires an extraction `partition-contract` resource,
but declares no resource shape or selection rule. The existing native chunk owner
already supplies the closed `source_blocks_v1` partition identity; ADR 0016 supplies
the compiled composition and derived part schemas. Adding a second partition
configuration would duplicate that authority. The snapshot writer and Spec state
reader currently receive neither the complete compiled extraction binding nor a
resolver for it. Ordinary snapshot validation cannot establish this missing join.

**Smallest proposed amendment to §12.7:** derive persisted extraction-contract
metadata from the existing compiled extraction bindings and native partition
identity, instead of introducing the undeclared partition resource. Bind workflow
ID/version, extraction node identity, `model-request/v1`, complete result-schema
resource, composition resource and part-operation bindings. Compare exact captured
contract content as well as IDs so changing a resource under the same ID/version
cannot silently select a newer contract. Persisted metadata is comparison evidence,
never a schema/executable fallback or saved model request.

The compiler/registry remains the single compiled authority. The snapshot writer
records its typed projection; the existing state-reader/publication validation path
resolves it against that authority and rejects unavailable or changed contracts
with `REFERENCE_EXTRACTION_CONTRACT_UNAVAILABLE`. Keep source/claim/token validation
and fresh-start semantics. Cover missing metadata, changed schemas/part dependencies,
foreign workflow/version/node, unchanged readback, and release of execution state
before readback. Do not implement the separate feedback API or array partitioning.

This focused design choice requires approval under AGENTS.md Project authority.
The approved §22.6 correction amendment does not resolve it; full readiness and a
Chunk 19 live-run proposal remain blocked until it is implemented and verified.
...
- **Chunk 18 — R36 logical slices are the next requested response change.**
  ...
  needed before validated slices can replace the combined global response. Existing
  retry, assembly and relationship owners remain authoritative; persisted readback
  and live publication/scoring remain independent closure requirements.
...
- **Graph capacity:** preparation/admission consolidation originally compiled to
  498/512 operations. The subsequent approved response composition compiles to
  519/1,024; it retains explicit child execution and independent retry identities.
...
**Next: [persisted extraction-contract readback](#persisted-extraction-contract--focused-decision-required).**
R35's recovery/exhaustion regression and shared correction simplification are
delivered. Resolve the readback authority decision, implement and verify it before
seeking Chunk 19 approval. Whole-array parts cannot simplify the failed summary;
item-level decomposition remains a separate decision if selected.
