...
pub const Action = struct {
    sink: sink_port.StreamCloser,
    pub const contract: pipeline.NodeContract = .{
        .id = "close-feature-log-stream",
        .kind = .action,
    ...
    pub fn execute(self: Action, binding: *const log_binding.ValidatedFeatureLogBinding, stream: log_stream.Stream, state: log_stream.StreamState, trailer: []const u8) Error!void {
