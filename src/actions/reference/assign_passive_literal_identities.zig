...
pub const Action = struct {
    pub const contract: pipeline.NodeContract = .{ .id = "assign-passive-literal-identities@1", .kind = .action, .requires = &.{.passive_literal_candidates}, .produces = &.{.passive_literal_identities}, .side_effect = .none };
