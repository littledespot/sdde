...
pub const Action = struct {
    pub const contract: pipeline.NodeContract = .{ .id = "assign-preserved-token-identities", .kind = .action, .requires = &.{.classified_reference_tokens}, .produces = &.{.preserved_token_identities}, .side_effect = .none };
