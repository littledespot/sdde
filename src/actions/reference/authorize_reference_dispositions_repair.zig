...
pub const Action = struct {
    pub const contract: pipeline.NodeContract = .{
        .id = "authorize-reference-dispositions-repair",
        .kind = .action,
        .requires = &.{ .parsed_reference_reconciliation, .citable_reference_inputs, .reference_passive_literals, .valid_toolchain, .validated_reference_dispositions },
        .produces = &.{.reference_reconciliation_repair},
        .invalidates = &.{.validated_reference_dispositions},
        .side_effect = .none,
    };
