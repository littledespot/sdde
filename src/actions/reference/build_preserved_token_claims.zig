...
pub const Action = struct {
    ...
    pub fn execute(_: Action, allocator: std.mem.Allocator, assigned: extraction.TokenAssignments) extraction.Error!extraction.Prepared {
        ...
        for (assigned.classified.text_validated.entries, entries) |entry, *result| {
            ...
            var classifications: std.ArrayList(extraction.tokens.Classification) = .empty;
            // The checked selections follow source-candidate order, independent
            // of the model's classification ordering.
            for (assigned.classified.selections) |selected| {
                if (!selected.candidate.fact.scope.chunk_id.eql(entry.scope.chunk_id)) continue;
                const classification: extraction.tokens.Classification = switch (selected.decision) {
                    .preserve => |kind| .{ .preserve = .{ .token_candidate_id = selected.candidate.id, .kind = kind } },
                    .irrelevant => .{ .irrelevant = selected.candidate.id },
                    .blocked => continue,
                };
            ...
            result.* = .{ .scope = entry.scope, .token_classifications = try classifications.toOwnedSlice(allocator), .outcome = switch (entry.outcome) {
