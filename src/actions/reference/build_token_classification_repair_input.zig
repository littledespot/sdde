...
pub const Action = struct {
    pub const contract: pipeline.NodeContract = .{ .id = "build-token-classification-repair-input", .kind = .action, .requires = &.{ .citable_reference_inputs, .reference_passive_literals, .structured_token_candidates, .text_validated_reference_extraction, .token_classification_repair_authorization }, .produces = &.{.model_input_packet}, .side_effect = .none };
