...
pub const Action = struct {
    ...
    pub fn execute(_: Action, allocator: std.mem.Allocator, progress: iteration.Progress, packet: *const packets.Packet, body: []const u8, origin: ?@import("../../domain/model_candidate_origin.zig").Origin) @import("../../domain/reference_extraction.zig").Error!iteration.Progress {
        ...
        return iteration.append(allocator, progress, .{ .state_id = .{ .bytes = unit.reference_chunk.reference_state_id.bytes }, .chunk_id = .{ .bytes = unit.reference_chunk.chunk_id.bytes } }, body, origin);
    }
};
