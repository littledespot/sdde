...
pub const Action = struct {
    pub const contract: pipeline.NodeContract = .{ .id = "parse-reference-extraction-repair", .kind = .action, .requires = &.{ .reference_extraction_repair_authorization, .model_input_packet, .model_request_identity_ledger, .prepared_model_request, .accounted_model_attempt, .terminal_provider_operation, .model_payload_schema_result }, .produces = &.{.reference_extraction_repair_result}, .side_effect = .none };
