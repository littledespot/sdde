...
pub const Action = struct {
    pub const contract: pipeline.NodeContract = .{ .id = "validate-reference-extraction-text@1", .kind = .action, .requires = &.{ .parsed_reference_extraction, .reference_passive_literals, .valid_toolchain, .citable_reference_inputs }, .produces = &.{.text_validated_reference_extraction}, .side_effect = .none };
