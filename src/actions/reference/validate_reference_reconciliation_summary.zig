...
pub const Action = struct {
    ...
    pub fn execute(self: Action, allocator: std.mem.Allocator, parsed: r.Parsed, context: validation.TextContext) r.Error!d.Result(r.CheckedSummary) {
        ...
        r.sameSet(r.ClaimId, proposal.member_claim_ids, parsed.input.partition.group.claim_ids) catch return d.reject(r.CheckedSummary, parsed.input, parsed.source, .summary, .{ .rule = .membership, .observed = .{ .claims = proposal.member_claim_ids }, .expected = .{ .claims = parsed.input.partition.group.claim_ids } });
        r.sameSet(r.SummaryId, proposal.member_summary_ids, parsed.input.member_summary_ids) catch return d.reject(r.CheckedSummary, parsed.input, parsed.source, .summary, .{ .rule = .membership, .observed = .{ .summaries = proposal.member_summary_ids }, .expected = .{ .summaries = parsed.input.member_summary_ids } });
        ...
        for (proposal.statements, statements, 0..) |statement, *result, index| {
            ...
            if (validation.claims(parsed.input.progress.plan.layout.items, statement.claim_ids, proposal.member_claim_ids)) |issue| return d.reject(r.CheckedSummary, parsed.input, parsed.source, .{ .statement = index }, issue);
        ...
        r.sameSet(r.ClaimId, represented.items, proposal.member_claim_ids) catch return d.reject(r.CheckedSummary, parsed.input, parsed.source, .summary, .{ .rule = .membership, .observed = .{ .claims = represented.items }, .expected = .{ .claims = proposal.member_claim_ids } });
