...
pub const Action = struct {
    ...
    pub fn execute(_: Action, comptime purpose: support.Purpose, allocator: std.mem.Allocator, inputs: authority.Inputs, context: p.Context, candidate: support.Contract(purpose).Candidate, authorization: repair.Contract(purpose).Authorization) repair.Contract(purpose).Error!*@import("../../domain/model_input_packet.zig").Packet {
        return repair.Contract(purpose).packet(allocator, inputs, context, candidate, authorization);
    }
};
