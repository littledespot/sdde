...
pub const Action = struct {
    ...
    pub fn execute(_: Action, allocator: std.mem.Allocator, current: session.Session, candidate: repair.Candidate, authorization: repair.Authorization, replacement: repair.Replacement, origin: ?@import("../../domain/model_candidate_origin.zig").Origin) repair.Error!repair.Candidate {
        return repair.merge(allocator, current, candidate, authorization, replacement, origin);
    }
};
