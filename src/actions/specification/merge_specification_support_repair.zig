...
pub const Action = struct {
    ...
    pub fn execute(_: Action, allocator: std.mem.Allocator, inputs: authority.Inputs, context: p.Context, candidate: support.Candidate, state: repair.State) repair.Error!support.Collection {
        return repair.merge(allocator, inputs, context, candidate, state.authorization, if (state.response) |response| response.value else null, if (state.response) |response| response.origin else null);
    }
};
