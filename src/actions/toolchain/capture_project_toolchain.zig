...
pub const Action = struct {
    ...
    pub const contract: pipeline.NodeContract = .{ .id = "capture-project-toolchain", .kind = .action, .requires = &.{}, .produces = &.{.project_toolchain_capture}, .side_effect = .filesystem_read };
