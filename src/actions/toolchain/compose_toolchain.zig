...
pub const Action = struct {
    pub const contract: pipeline.NodeContract = .{ .id = "compose-toolchain", .kind = .action, .requires = &.{ .schema_valid_project_toolchain, .resolved_toolchain_inheritance }, .produces = &.{.composed_toolchain}, .side_effect = .none };
