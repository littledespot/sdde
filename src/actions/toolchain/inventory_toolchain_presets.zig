...
pub const Action = struct {
    ...
    pub const contract: pipeline.NodeContract = .{ .id = "inventory-toolchain-presets", .kind = .action, .requires = &.{}, .produces = &.{.toolchain_preset_inventory}, .side_effect = .filesystem_read };
