...
pub const Action = struct {
    ...
    pub const contract: pipeline.NodeContract = .{ .id = "parse-toolchain-documents", .kind = .action, .requires = &.{ .project_toolchain_capture, .toolchain_preset_captures }, .produces = &.{.raw_toolchain_documents}, .side_effect = .none };
