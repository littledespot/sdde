...
pub const Action = struct {
    pub const contract: pipeline.NodeContract = .{ .id = "validate-toolchain-preset-registry", .kind = .action, .requires = &.{.raw_toolchain_documents}, .produces = &.{.schema_valid_toolchain_registry}, .side_effect = .none };
