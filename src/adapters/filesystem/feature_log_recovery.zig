...
fn inspectSegment(
    ...
    try format.validateEncodedRow(allocator, header_with_lf, if (stream == .event) format.event_column_count else format.prompt_column_count);
    ...
    while (lines.next()) |line| {
        ...
        try format.validateEncodedRow(allocator, with_lf, if (stream == .event) format.event_column_count else format.prompt_column_count);
