...
pub const Adapter = struct {
    ...
    pub fn port(self: *Adapter) @import("../../ports/workflow_output.zig").Port {
    ...
    fn publish(context: *anyopaque, capability: *const roots.FeatureOutputWriteCapability, allocator: std.mem.Allocator, prepared: output.Prepared) output.Error!void {
        ...
        for (prepared.files) |file| {
            ...
            files.replace(self.io, a, parent, std.fs.path.basename(path.root_relative), condition, file.bytes) catch return error.OutputWriteFailed;
            switch (file.target) {
