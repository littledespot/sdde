            ...
            .claims => |proposals| claims: {
                ...
                for (proposals, checked, 0..) |proposal, *value, index| {
                    ...
                    value.content = switch (proposal.content) {
                        inline .business, .scope_guard => |candidate_text, tag| @unionInit(extraction.Content, @tagName(tag), switch (try validator.checkBusinessIn(allocator, context, candidate_text)) {
                        ...
                        inline else => |candidate_text, tag| @unionInit(extraction.Content, @tagName(tag), switch (try validator.checkReferenceIn(allocator, context, candidate_text)) {
                            .valid => |accepted| accepted,
                            .invalid => |issue| return reject(allocator, inputs, registry, current, parsed, candidate, .{ .claim = index }, issue, .{ .content = proposal.content }),
                        }),
                    };
