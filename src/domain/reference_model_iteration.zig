...
pub fn append(allocator: std.mem.Allocator, progress: Progress, scope: source.Scope, bytes: []const u8, origin: ?@import("model_candidate_origin.zig").Origin, producers: ?e.ProducerOrigins) e.Error!Progress {
    ...
    entry.* = .{ .value = .{ .scope = expected, .origin = origin, .producers = producers, .result = .{ .response = try allocator.dupe(u8, bytes) } }, .previous = progress.latest };
