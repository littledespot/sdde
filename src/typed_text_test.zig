...
test "all extraction content kinds and no-claim reasons pass through the shared text gate" {
    ...
    try std.testing.expectEqual(.unbound_path, (try fixture.validate_text.execute(allocator, prepared.registry, fixture.safety.value(prepared.owner), inputs, parsed)).invalid.issue.reason);
}
