...
pub const Publication = enum { not_observed, confirmed };
pub const Result = struct { status: c.Status, missing_artifact: ?c.Artifact = null, specification: ?[]const u8 = null };
...
pub fn inspect(io: std.Io, allocator: std.mem.Allocator, project: std.Io.Dir, outcome: @import("../../../src/domain/workflow.zig").OutcomeTag, publication: Publication, expected: []const c.Artifact, resolved: paths.FeaturePaths) !Result {
    ...
    return .{ .status = .passed, .specification = resolved.get(.specification).project_relative };
}
